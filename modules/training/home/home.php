                ...
                if (result.code == 200) {
                    result.data.forEach(function (course) {
                        course_list += '<div class="col-lg-3 mt-3">' +
                            ...
                            '<a href="index.php?m=shopcart&a=send_mail&name=' + course.name + '" class="btn btn-info" style="float:right;margin:10px;">Quan Tâm </a>' +
                            '<div class="card-body">' +
                            '<span class="badge badge-secondary product-method">Online</span>' +
                            '<h5 class="card-title">'+ course.name +'</h5>' +
                            '<p class="card-text product-price">'+ $.number(course.price, 0, ",", ".") +' ₫</p>' +
                            '<a href="index.php?m=shopcart&a=cart&id=' + course.id + '" class="btn btn-danger product-btn-buy">Đăng Kí Học </a>' +
                            '</div>' +
                            '</div>' +
                            '</div>';
                    });
